...
(function (global, factory) {
	...
	const helpVec3_1 = new THREE__namespace.Vector3();
	const helpVec3_2 = new THREE__namespace.Vector3();
	const helpVec3_3 = new THREE__namespace.Vector3();
	const helpMat4 = new THREE__namespace.Matrix4();
	const helpCurve = new THREE__namespace.QuadraticBezierCurve3();
	 ...
	 * PathGeometry
	 */
	class PathGeometry extends THREE__namespace.BufferGeometry {
		/**
		...
		_resizeAttribute(name, len) {
			...
			while (attribute.array.length < len) {
				...
				const newAttribute = new THREE__namespace.BufferAttribute(new Float32Array(oldLength * 2), attribute.itemSize, attribute.normalized);
		...
		_resizeIndex(len) {
			...
			while (index.array.length < len) {
				...
				const newIndex = new THREE__namespace.BufferAttribute(oldLength * 2 > 65535 ? new Uint32Array(oldLength * 2) : new Uint16Array(oldLength * 2), 1);
	...
	function generatePathVertexData(pathPointList, options, generateUv2 = false) {
		...
		const indices = [];
		let verticesCount = 0;
		const right = new THREE__namespace.Vector3();
		const left = new THREE__namespace.Vector3();

		// for sharp corners
		const leftOffset = new THREE__namespace.Vector3();
		const rightOffset = new THREE__namespace.Vector3();
		const tempPoint1 = new THREE__namespace.Vector3();
		const tempPoint2 = new THREE__namespace.Vector3();
		...
		const sharp = new THREE__namespace.Vector3();
	...
	function generateTubeVertexData(pathPointList, options, generateUv2 = false) {
		...
		const indices = [];
		let verticesCount = 0;
		const normalDir = new THREE__namespace.Vector3();
